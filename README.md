...
Latest stable: [DS Agent v0.8.0](https://github.com/Lee-take/dsagent/releases/tag/v0.8.0)
...
## 0.8.0 Stable Status / 0.8.0 正式版状态

Version `0.8.0` is the current Windows-first stable release. It adds Durable
...
`0.8.0` 是当前 Windows 优先正式版本。它新增持久、可验证的
...
`v0.8.0` is the current Windows stable line. The published `v0.8.0-rc.1`
prerelease remains immutable historical update-test evidence.
...
`v0.8.0` keeps the verifiable Windows Agent foundation: install DS Agent, connect
...
`v0.8.0` 延续可验证的 Windows Agent 工作：安装 DS Agent，通过本地环境变量连接
