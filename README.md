...
Latest release: [DS Agent v0.4.1](https://github.com/Lee-take/dsagent/releases/tag/v0.4.1)
...
## 0.4.1 Status / 0.4.1 状态

Version `0.4.1` is the current Windows-first formal release. It includes native
...
`0.4.1` 是当前 Windows 优先正式发布版本。它包含原生 Subagent 并行工作：父任务可以
...
The public `v0.0.1` through `v0.4.0` releases remain unchanged. The `v0.4.1`
...
`v0.4.1` keeps the verifiable Windows Agent foundation: install DS Agent, connect
...
`v0.4.1` 延续可验证的 Windows Agent 工作：安装 DS Agent，通过本地环境变量连接
...
example `release/bundle/nsis/DS Agent_0.4.1_x64-setup.exe`. The Windows config
...
The current 0.4.1 preview includes the permission loop for built-in local tools,
